...
def main(imgpath):
    ...
    X_color, T_gray, T_vgg = util.img_to_tensor(pil_img=pil_img)
    X_color, T_gray, T_vgg = X_color.unsqueeze(0), T_gray.unsqueeze(0), T_vgg.unsqueeze(0)
    Y_grayscale, Y_restored = invertible_grayscale(X_color)
    print(Y_grayscale.size(), Y_restored.size())
    loss = criterion(X_color, T_gray, T_vgg, Y_grayscale, Y_restored, stage=1)
    print(loss)
    loss.backward()
